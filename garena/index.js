		...
		try {
			...
			switch (command) {
				...
				case 'ytmp3':
					...
					anu = await fetchJson(`http://kocakz.herokuapp.com/api/media/ytaudio?url=${args[0]}`, { method: 'get' })
					if (anu.error) return reply(anu.error)
					teks = `❏ *Titulo* : ${anu.title}\n❏ *Tamanho* : ${anu.filesize}\n\nCarregando poha aguarda..`
				...
				case 'mp3':
					...
					reply(mess.wait)
					play = body.slice(5)
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/yta?url=${play}&apikey=BotWeA`)
					if (anu.error) return reply(anu.error)
					infomp3 = `*Musica Encontrada*\nTitulo : ${anu.result.title}\nLink : ${anu.result.source}\n\n*ESPERE ENVIANDO POR FAVOR*`
					buffer = await getBuffer(anu.result.thumbnail)
					lagu = await getBuffer(anu.result.url_audio)
